...
# ROOT_DIR="D:/pycharmProjects/Detect2/"
ROOT_DIR=os.path.abspath('.')+'\\'
print(ROOT_DIR)
...
from mrcnn import model_noise as modellib
...
def detect(model, dataset_dir, subset):
    ...
    submit_dir = os.path.join(RESULTS_DIR, 'one')
...
def detect_zip(model, dataset_dir, subset):
    """Run detection on images in the given directory."""
    print("Running on {}".format(dataset_dir))
    ...
    # submit_dir = "submit_{:%Y%m%dT%H%M%S}".format(datetime.datetime.now())
    # submit_dir = os.path.join(RESULTS_DIR, submit_dir)
    submit_dir = os.path.join(RESULTS_DIR, 'multi')
    if not os.path.exists(submit_dir):
        os.makedirs(submit_dir)
    ...
    for image_id in dataset.image_ids:
        ...
        print(dataset.image_info[image_id]["source"])
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        detect_zip(model,args.dataset, args.subset)
